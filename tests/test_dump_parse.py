...
class TestDumpParse(unittest.TestCase):
    def test_dump_defaults_to_sp(self):
        ...
        args, error = _parse_args([], AddressResolver(regs))
    ...
    def test_dump_addr_and_len(self):
        regs = {}
        args, error = _parse_args(["0x3000", "32"], AddressResolver(regs))
        self.assertIsNone(error)
    ...
    def test_dump_flags(self):
        regs = {"sp": 0x1000}
        args, error = _parse_args(["-l", "128", "-w", "8"], AddressResolver(regs))
        self.assertIsNone(error)
        self.assertEqual(args.addr, 0x1000)
    ...
    def test_dump_invalid_width(self):
        regs = {"sp": 0x1000}
        _, error = _parse_args(["-w", "0"], AddressResolver(regs))
        self.assertEqual(error, "invalid width value")

    def test_dump_invalid_length(self):
        regs = {"sp": 0x1000}
        _, error = _parse_args(["-l", "0"], AddressResolver(regs))
        self.assertEqual(error, "invalid length value")

    def test_word_dump_defaults(self):
        regs = {"sp": 0x1000}
        args, error = _parse_simple_args([], AddressResolver(regs))
        self.assertIsNone(error)
        self.assertEqual(args.addr, 0x1000)

    def test_word_dump_len(self):
        regs = {"sp": 0x1000}
        args, error = _parse_simple_args(["0x2000", "64"], AddressResolver(regs))
        self.assertIsNone(error)
        self.assertEqual(args.addr, 0x2000)
